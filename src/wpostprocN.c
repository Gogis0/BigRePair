...

{
	...
	/*for (i = 0; i < diczCexpLen; i++) {
		e = fread(val, sizeof(int), 1, R);
		die(e != 1, "Read error");
		if (val[0] >= 256)
			val[0] += 2 * Rlen;
		fseek(R, -1 * sizeof(int), SEEK_CUR);
		fwrite(val, sizeof(int), 1, R);
	}*/
	fprintf(stderr, "end: %d\n", ftell(R) / 4);
	for (i = 0; i < p; i++) {
		if (transl[i] < 256)
	...
	//update P_1 terminals in R_D_2 and C_D_2 to new positions that are in R (C_D_1 area = transl)
	/*fprintf(stderr, "end: %d\n", ftell(R) / 4);
	if (fclose(R) != 0) {
		fprintf(stderr, "Cannot close %s.R\n", argv[1]); exit(1);
	}
	sprintf(fname, "%s.R", argv[1]);
	R = fopen(fname, "rw"); // output R file*/
